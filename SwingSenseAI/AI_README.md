...
에이전트가 터미널 명령어를 실행할 때는 반드시 이 디렉토리(`SwingSenseAI/`)에서 아래 명령어를 기준으로 실행해야 합니다.
...
- **계측 테스트 실행 (실기기/에뮬레이터 연결 필요)**:
  ...
  ```
  > 참고: CI/에이전트 환경에 기기가 없으면 계측 테스트는 건너뛰고 단위 테스트(`./gradlew test`) 결과로 검증합니다.
