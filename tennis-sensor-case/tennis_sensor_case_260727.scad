...
// Must exceed lip rebate depth (lip_w+lip_gap=1.15mm) to reach outer wall solid.
// Using side_wall_w ensures full penetration; clip-to-dome trims the excess.
eps = side_wall_w;
...
module board_brackets() {
    ...
    for (c = corners) {
        ...
        // Rib: extend eps outward into the shell wall
        rib_x = (xd > 0 ? xw - eps : xw - bw);  // extend toward wall
        rib_y = (yd > 0 ? yp        : yp - bw);
        rib_w = bw + eps;  // wider to penetrate wall
        translate([rib_x, rib_y, 0])
            cube([rib_w, bw, shelf_z + shelf_thick]);
        // Lip: extend eps outward into the shell wall
        lip_x = (xd > 0 ? xw - eps : xw - bl);  // extend toward wall
        translate([lip_x, rib_y, shelf_z])
            cube([bl + eps, bw, shelf_thick]);
    }
}
...
module switch_bay_walls() {
    ...
    t  = sw_wall_t;
    h  = brd_h * 0.7;
    // Extend side walls eps into the +Y outer wall so they merge with the shell
    translate([-sw_w/2 - t, by,     shelf_z]) cube([t, sw_bay_depth + eps, h]);
    translate([ sw_w/2,     by,     shelf_z]) cube([t, sw_bay_depth + eps, h]);
