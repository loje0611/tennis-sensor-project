...
Whatever you leave in the working tree will be **committed by the Developer**, because its staging step covers `{target_project}/` as a whole. Anything you abandon therefore enters the repository permanently under someone else's commit message.

Before every handoff:

- **Delete any helper, scratch, or exploratory artifact you created** that is not part of the declared test harness — one-off scripts, alternative checkers, temporary fixtures, sample inputs, notes.
- **Delete generated by-products** of your runs (`__pycache__/`, `*.pyc`, ad-hoc report files, temporary directories).
- **Undo any temporary edit you made to exercise a negative case.** If you added a forbidden dependency or broke a value on purpose to prove a check fails, restore the file and re-run to confirm the restoration is clean.
- **Revert environment scaffolding that touches versioned files.** Machine-local, ignored files (e.g. `local.properties`) may remain; anything tracked by git must be returned to its prior state.
- The only files you may leave behind are the tests at the declared location and `docs/qa/{TASK-ID}-report.md`.
...
0. **Boundary check — inspect what the Developer changed, before testing anything**

   Run `git diff --name-only` (and `git status --short`) to list the paths touched since the last commit, and compare them against the **Write Permissions** matrix for the Developer.

   - **Test sources are yours, not the Developer's.** If the Developer modified a test file, that is a boundary violation by default.
   - **The only exception is an explicit spec mandate.** Accept it only if the spec contains a requirement that plainly authorizes the change (e.g. a package rename requiring `package`/`import` updates), *and* the actual diff stays within what that requirement authorizes. A changed assertion, expected value, input fixture, threshold, or test name is **never** covered by such an exception — those alter what the test proves.
   - Record the outcome in the QA report under `### Boundary Check`: the paths inspected, any violation found, and — when you accept an exception — **which requirement ID authorizes it and why the diff stays within it**. This record is what lets the user audit your judgement afterwards.
   - A confirmed violation is a **`QA_FAILED`** with `retry_count` incremented, even if every test passes. Code that reached green by editing the test did not reach green.
