...
기존 카메라 미리보기 화면 위에 (1) 정답 드릴 선택 바(`DrillType`), (2) 센서 연결 및 세션 시작/종료 제어 버튼, (3) 스윙 직후 갱신되는 실시간 라켓 페이스 각도(`OPEN`/`CLOSED`/`SQUARE`), (4) 5단계 통합 운동 체인 시각화 바, (5) 인과 코칭 피드백 및 통계적 이상치(피로도) 알림 카드를 결합하여 사용자에게 즉각적이고 과학적인 스윙 피드백을 제공합니다.
...
- `:feature:lab` 모듈 UI 컴포넌트 구현 (`io.github.loje0611.tennisdoc.feature.lab.ui`):
  ...
  - `LabSessionControlHeader`: BLE 연결 상태 뱃지("센서 연결됨" / "센서 찾는 중..." / "센서 미연결"), 센서 연결/취소 버튼, 세션 타이머, 스윙 카운터, 측정 시작/종료 액션 버튼.
  ...
  - `LabScreen`: 카메라 권한(`CAMERA`) 및 BLE 런타임 권한(`BLUETOOTH_SCAN`, `BLUETOOTH_CONNECT`, `ACCESS_FINE_LOCATION`, `POST_NOTIFICATIONS`) 요청 및 오버레이 배선.
...
- 단위 테스트 및 Compose 렌더링 검증:
  - 드릴 선택, 센서 연결/취소, 세션 시작/종료 시 상태 전이 테스트.
  - 융합 결과 인입 시 피드백 카드 상태 바인딩 테스트.
  - 센서 미연결 시 세션 시작 거부 및 토스트 안내 검증.
...
- **센서 연결 상태 및 제어**:
  - `isSensorConnected == true`: "센서 연결됨"(녹색 원 뱃지) 표시. 센서 연결/취소 버튼 숨김.
  - `isSensorScanning == true`: "센서 찾는 중..."(황색 원 뱃지) 표시. "연결 취소" 버튼 표시 ➔ 클릭 시 `disconnectSensor()` 호출.
  - `isSensorConnected == false` && `isSensorScanning == false`: "센서 미연결"(적색 원 뱃지) 표시. "센서 연결" 버튼 표시 ➔ 클릭 시 BLE 권한 확인 후 `connectSensor()` 호출.
- **세션 미시작 상태 (`isSessionActive == false`)**:
  - 선택된 드릴 이름("목표: {드릴명}") 표시.
  - "측정 시작" 버튼 활성화 ➔ 클릭 시 센서 연결 여부를 확인하고, 연결된 경우 `startSession(SessionType.LAB, selectedDrill)` 호출 후 세션 활성 상태로 전이. 센서 미연결 시 세션을 시작하지 않고 FR-7의 안내를 수행한다.
- **세션 실행 중 상태 (`isSessionActive == true`)**:
  - 실시간 세션 경과 시간(MM:SS), 누적 스윙 수 표시 ("00:00 | 스윙 0회").
  - "측정 종료" 버튼(적색) ➔ 클릭 시 `finishSession()` 호출 및 세션 종료.
...
- **조작 편의성**: 한 손으로도 드릴 변경, 센서 연결/취소, 세션 시작/종료가 가능하도록 큼직한 터치 타겟(최소 48dp) 확보.
- **권한 처리 UX**: Lab 화면 진입 시 카메라 권한(`CAMERA`) 및 OS 버전별 BLE 런타임 권한(`BLUETOOTH_SCAN`, `BLUETOOTH_CONNECT`, `ACCESS_FINE_LOCATION`, `POST_NOTIFICATIONS`)을 순차적으로 요청한다.
...
- [x] **AC-1**: `DrillSelectorBar`, `LabSessionControlHeader`, `LabRealtimeFeedbackCard`, `LabAnomalyAlertBanner` 컴포저블이 `:feature:lab`에 구현된다.
- [x] **AC-2**: 드릴 선택기에서 구종 변경 시 `selectedDrill` 상태가 갱신되고, 세션 시작 시 변경이 비활성화된다.
- [x] **AC-3**: "측정 시작" 클릭 시 `startSession(SessionType.LAB, drillType)`이 호출되고 "측정 종료"로 토글된다.
- [x] **AC-4**: `FusedSwing` 데이터 인입 시 하단 카드에 라켓 페이스 상태(`OPEN`/`CLOSED`/`SQUARE`), 5단계 체인 게이지, 인과 코칭 문구가 렌더링된다.
- [x] **AC-5**: 피로도/이상치 리포트 인입 시 상단 경고 배너가 표시된다.
- [x] **AC-6**: `LabViewModelTest` 및 UI 단위 테스트가 100% 통과한다.
- [x] **AC-7**: `./gradlew :feature:lab:test :app:testDebugUnitTest verifyModuleDependencies :app:assembleDebug` 명령이 0 Failures로 통과한다.
- [x] **AC-8**: 센서 미연결 상태에서 헤더에 "센서 미연결"과 "센서 연결" 버튼이 표시되고, 클릭 시 센서 검색 상태(`isSensorScanning == true`)로 전환되며 "연결 취소" 버튼이 표시된다.
- [x] **AC-9**: 센서 미연결 상태에서 "측정 시작" 클릭 시 `startSession()`이 실패(`false`)하고 세션이 시작되지 않으며 "센서를 먼저 연결해 주세요" Toast가 출력된다.
