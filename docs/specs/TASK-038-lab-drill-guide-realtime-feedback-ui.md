...
| v4 | 2026-08-15 | PM | 사용자 피드백 반영: (1) 초심자 타겟 드릴 목록 5종 단순화(`FOREHAND`, `BACKHAND`, `SERVE`, `FOREHAND_VOLLEY`, `BACKHAND_VOLLEY`), (2) 포그라운드 서비스 및 상주 알림 라이프사이클을 `isSessionActive` 세션 실행 중에만 한정하도록 개정 (AC-11 신설) |
...
테니스 입문자/초심자의 멘탈 모델에 최적화하여 (1) 직관적인 5대 기본 드릴 선택 바(`DrillType`), (2) 센서 연결 상태 인디케이터 및 단일 [측정 시작/종료] 버튼, (3) 스윙 직후 갱신되는 실시간 라켓 페이스 각도(`OPEN`/`CLOSED`/`SQUARE`), (4) 5단계 통합 운동 체인 시각화 바, (5) 인과 코칭 피드백 및 통계적 이상치(피로도) 알림 카드를 결합하여 사용자에게 직관적이고 과학적인 스윙 피드백을 제공합니다. 또한 백그라운드 포그라운드 서비스 및 알림창은 세션 실행 중에만 한정하여 비정상 알림 노출을 차단합니다.

### 1.2 범위
- `:feature:lab` 모듈 UI 컴포넌트 구현 (`io.github.loje0611.tennisdoc.feature.lab.ui`):
  - `DrillSelectorBar`: 초심자 친화적 5대 드릴 선택 칩 (`FOREHAND`, `BACKHAND`, `SERVE`, `FOREHAND_VOLLEY`, `BACKHAND_VOLLEY`).
  - `LabSessionControlHeader`: BLE 센서 연결 상태 뱃지("센서 연결됨" / "센서 찾는 중..." / "센서 미연결"), 세션 타이머, 스윙 카운터, **단일 [측정 시작 / 측정 종료] 액션 버튼**.
  ...
  - `LabScreen`:
    ...
    - **개발자 모드(`isDebugModeEnabled == true`) 활성화 시에만 FPS/ms 성능 오버레이 표시**.
    - 화면 이탈 시 세션 미실행 상태(`!isSessionActive`)이면 센서 리소스 및 서비스 정리.
- `DrillType` 모델 정비 (`:core:model`):
  - 기존 세부 스핀 구종(`FOREHAND_FLAT`, `FOREHAND_TOPSPIN`, `FOREHAND_SLICE`, `BACKHAND_FLAT`, `BACKHAND_TOPSPIN`, `BACKHAND_SLICE`, `VOLLEY`)을 **5대 드릴(`FOREHAND`, `BACKHAND`, `SERVE`, `FOREHAND_VOLLEY`, `BACKHAND_VOLLEY`)**로 정비.
- `SwingAnalysisForegroundService` 및 세션 라이프사이클:
  - 단순 센서 연결/대기 중에는 상주 알림을 표시하지 않음.
  - `startSession()` 실행 시에만 포그라운드 상주 알림 활성화, `finishSession()` 시 알림 즉시 해제.
...
### 2.1 주요 정의
- **드릴 가이드 (Drill Guide)**: 초심자가 목표로 하는 기본 스윙 동작(포핸드, 백핸드, 서브, 포발리, 백발리)을 사전에 지정하는 훈련 단위.
- **포발리 / 백발리 (FH Volley / BH Volley)**: 포핸드 발리(`FOREHAND_VOLLEY`)와 백핸드 발리(`BACKHAND_VOLLEY`)의 일상적 통용 약칭.
- **세션 종속 포그라운드 알림 (Session-Scoped Notification)**: 앱이 백그라운드로 내려가도 측정이 지속되어야 하는 실제 측정 진행(`isSessionActive == true`) 중에만 표시되는 시스템 알림.
...
### FR-1: 초심자 최적화 드릴 선택기 (`DrillSelectorBar`)
- 사용자가 선택할 수 있는 드릴 목록을 5가지 기본 스윙으로 한정하여 수평 스크롤 칩으로 제공한다:
  1. `FOREHAND` ("포핸드") — 기본값
  2. `BACKHAND` ("백핸드")
  3. `SERVE` ("서브")
  4. `FOREHAND_VOLLEY` ("포발리" 또는 "포핸드 발리")
  5. `BACKHAND_VOLLEY` ("백발리" 또는 "백핸드 발리")
- 세션이 실행 중(`isSessionActive == true`)일 때는 드릴 칩 변경을 비활성화한다.

### FR-2: 세션 시작/종료 제어 및 헤더 (`LabSessionControlHeader`)
- **센서 연결 상태 인디케이터**:
  - `isSensorConnected == true`: "센서 연결됨"(녹색 원 뱃지)
  - `isSensorScanning == true`: "센서 찾는 중..."(황색 원 뱃지)
  - `isSensorConnected == false` && `isSensorScanning == false`: "센서 미연결"(적색 원 뱃지)
- **단일 세션 액션 버튼**:
  - **세션 미시작 상태 (`isSessionActive == false`)**:
    - 선택된 드릴 이름("목표: {드릴명}") 표시.
    - **[측정 시작] 버튼(녹색)** 제공.
    - 클릭 시 센서 연결 상태이면 `startSession(SessionType.LAB, selectedDrill)`을 호출하고 포그라운드 알림을 시작한다. 미연결 상태이면 측정을 시작하지 않고 `Toast("센서를 먼저 연결해 주세요")`를 표시하며 센서 연결을 시도한다.
  - **세션 실행 중 상태 (`isSessionActive == true`)**:
    - 실시간 세션 경과 시간(MM:SS), 누적 스윙 수 표시 ("00:00 | 스윙 0회").
    - **[측정 종료] 버튼(적색)** 제공 ➔ 클릭 시 `finishSession()` 호출, 포그라운드 알림을 제거하고 세션을 마감한다.

### FR-3: 실시간 융합 피드백 카드 (`LabRealtimeFeedbackCard`)
- `latestFusedSwing` 데이터가 수신되면 화면 하단에 팝업 또는 고정 오버레이로 다음 정보를 렌더링한다:
  1. **라켓 페이스 상태**: `SQUARE`(녹색), `OPEN`(주황/적색), `CLOSED`(파랑/적색) 뱃지 및 각도 편차.
  2. **5단계 운동 체인 게이지**: 골반 ➔ 어깨 ➔ 손목 ➔ 라켓 ➔ 임팩트 순서 인디케이터 및 에너지 전달 효율 점수(%).
  3. **인과 코칭 텍스트**: 주 원인 및 인과 설명 (예: "상체 조기 개방으로 인한 페이스 열림 ➔ 골반 회전 선행 필요").

### FR-4: 이상치 및 피로도 알림 배너 (`LabAnomalyAlertBanner`)
- `latestAnomalyReport`의 `fatigue.isFatigued == true` 또는 `severity == CRITICAL`인 경우 상단 주의 배너를 표시한다.

### FR-5: 성능 디버그 오버레이 제어 (FPS/ms 지연)
- 실시간 카메라 프레임 처리 성능(FPS/ms) 칩은 **`isDebugModeEnabled == true`인 개발자 모드 상태에서만 화면 좌측 상단에 렌더링**하며, 일반 모드에서는 숨김(GONE) 처리한다.

### FR-6: 포그라운드 서비스 및 알림창 라이프사이클 관리
- 단순 센서 탐색/연결 대기 상태에서는 상주 알림을 띄우지 않는다.
- `startSession()`으로 실제 측정이 활성화될 때만 시스템 알림("스윙 분석이 실행 중입니다")을 포그라운드로 활성화한다.
- `finishSession()` 호출 시 상주 알림을 즉시 제거한다.
- 세션 미실행 상태(`isSessionActive == false`)에서 화면을 벗어나거나 앱이 백그라운드로 전환될 경우, 서비스를 안전하게 중지(`requestStop()`)하여 불필요한 백그라운드 배터리 및 알림 누수를 방지한다.
...
## 5. UI/UX 요구사항
- **시인성 및 가독성**: 카메라 실시간 뷰 위에 오버레이되므로 반투명 어두운 배경(Scrim, alpha 0.6~0.65)을 적용.
- **간결한 조작 인터페이스**: 단일 [측정 시작/종료] 버튼과 큰 터치 타겟(최소 48dp)을 제공.
- **초심자 맞춤 라벨링**: 드릴 칩 텍스트를 "포핸드", "백핸드", "서브", "포발리", "백발리"로 한눈에 명확하게 표기.
- **자동 연결 및 권한 플로우**: Lab 화면 진입 시 카메라 및 BLE 런타임 권한을 요청하고, 허용 시 자동으로 센서 연결을 시도함.
...
- **센서 미연결 상태에서 [측정 시작] 클릭 시**: `startSession()`은 `false`를 반환하여 세션을 시작하지 않고, `connectSensor()`를 백그라운드 호출하며 Toast("센서를 먼저 연결해 주세요")를 출력.
- **세션 미시작 상태에서 앱 이탈**: 포그라운드 서비스 및 알림이 실행되지 않거나 안전하게 종료되어 상주 알림 누수가 발생하지 않음.
...
- [ ] **AC-2**: `DrillType` 5종(`FOREHAND`, `BACKHAND`, `SERVE`, `FOREHAND_VOLLEY`, `BACKHAND_VOLLEY`)이 제공되고, UI 칩 선택 시 상태가 갱신되며 세션 시작 시 변경이 비활성화된다.
...
- [ ] **AC-8**: Lab 화면 진입 시 센서 자동 연결을 시도하며, `LabSessionControlHeader`에는 센서 상태 인디케이터(연결됨/찾는 중/미연결)와 단일 [측정 시작/종료] 버튼만 노출된다.
...
- [ ] **AC-11**: 측정이 시작되기 전(대기 상태)이나 세션 미실행 상태에서 앱을 이탈할 때는 "스윙 분석이 실행 중입니다" 상주 알림이 노출되지 않으며, `startSession()` 실행 중에만 상주 알림이 유지되고 `finishSession()` 시 알림이 즉시 닫힌다.
