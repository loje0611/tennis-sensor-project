...
- **자신의 문서 변경(spec, `task-board.json`, 계획 문서)은 핸드오프 전에 커밋**합니다. 미커밋 상태로 넘기면 Tester의 경계 검사(§6)가 이를 Developer의 무단 수정과 구별하지 못해 위반으로 보고하고 원복시킵니다.
...
- **에이전트는 자신이 만든 임시 산출물을 핸드오프 전에 스스로 치웁니다.** `{target_project}/` 전체가 스테이징 대상이므로 방치된 보조 스크립트·임시 픽스처·생성물 캐시(`__pycache__` 등)는 **다른 에이전트의 커밋 메시지 아래 영구 편입**됩니다. Developer는 커밋 직전 스테이징 목록에서 명세가 요구하지 않은 이런 파일을 걸러내고, 걸러낸 목록을 보고합니다.
- **Tester는 매 사이클 시작 시 Developer의 변경 경로를 쓰기 권한 매트릭스와 대조합니다(경계 검사).** Developer가 테스트 파일을 수정했다면 기본적으로 위반이며, **명세에 근거 조항이 있을 때만** 예외로 인정하고 그 요구사항 ID와 인정 사유를 QA 리포트에 기록합니다. 단정문·기대값·입력 픽스처·임계값 변경은 어떤 경우에도 예외가 아닙니다. 위반 확인 시 테스트가 전부 통과해도 `QA_FAILED`입니다.
- **자동 검사는 완전하지 않습니다.** 경계 검사는 Developer의 위반만 탐지하며, Tester 자신의 위반을 검사하는 주체는 없습니다. 기록(QA 리포트의 `### Boundary Check`)을 남기는 이유는 **사용자가 사후에 감사할 수 있게** 하기 위함이며, 최종 백스톱은 사용자입니다.
- **경계 검사는 경로로 판단할 뿐 작성자를 식별하지 못합니다.** `git diff`에는 누가 고쳤는지가 남지 않으므로, **커밋되지 않은 채 워킹 트리에 남은 PM의 문서 수정은 Developer의 무단 수정으로 보고되어 원복됩니다.** 이 때 Tester의 판정은 정당하며, 잘못은 커밋 없이 인계한 쪽에 있습니다. 그래서 각 에이전트는 **자신의 변경을 다음 에이전트가 기동하기 전에 커밋해 소유권을 확정**해야 합니다.
